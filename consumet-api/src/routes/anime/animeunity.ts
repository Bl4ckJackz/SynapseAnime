...
const routes = async (fastify: FastifyInstance, options: RegisterOptions) => {
  ...
  fastify.get('/', (_, rp) => {
    rp.status(200).send({
      intro: `Welcome to the animeunity provider: check out the provider's website @ ${animeunity.baseUrl}`,
  ...
  fastify.get('/recent-episodes', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      const res = await animeunity.fetchRecentEpisodes(page);
      reply.status(200).send(res);
    } catch (err) {
      reply.status(500).send({
        message: 'Something went wrong. Contact developer for help.',
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        const res = await animeunity.fetchEpisodeSources(episodeId);
        /*
        let res = redis
          ? await cache.fetch(
            redis as Redis,
