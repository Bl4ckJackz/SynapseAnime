...
export class JikanMangaService {
    ...
    async getMangaCharacters(malId: number): Promise<JikanCharactersResponse['data']> {
        const cacheKey = `jikan:manga:${malId}:characters`;

        const cached = this.cacheService.get<JikanCharactersResponse['data']>(cacheKey);
        if (cached) {
            return cached;
        }
    ...
    async getMangaStatistics(malId: number): Promise<JikanStatisticsResponse['data']> {
        const cacheKey = `jikan:manga:${malId}:statistics`;

        const cached = this.cacheService.get<JikanStatisticsResponse['data']>(cacheKey);
        if (cached) {
            return cached;
        }
    ...
    async getMangaRecommendations(malId: number): Promise<JikanRecommendationsResponse['data']> {
        const cacheKey = `jikan:manga:${malId}:recommendations`;

        const cached = this.cacheService.get<JikanRecommendationsResponse['data']>(cacheKey);
        if (cached) {
            return cached;
        }
    ...
    async getGenres(): Promise<Array<{ mal_id: number; name: string; count: number }>> {
        const cacheKey = 'jikan:genres';

        const cached = this.cacheService.get<Array<{ mal_id: number; name: string; count: number }>>(cacheKey);
        if (cached) {
            return cached;
        }

        const result = await this.executeWithRetry<{ data: Array<{ mal_id: number; name: string; count: number }> }>(
            () => this.client.get('genres/manga'),
            cacheKey,
        );
            ...
            async () => {
                ...
                for (let attempt = 1; attempt <= maxRetries; attempt++) {
                    try {
                        ...
                        const responseEtag = (response as unknown as { headers?: Record<string, string> }).headers?.['etag'];
                        if (responseEtag) {
                            this.cacheService.set(cacheKey, response.data, this.serviceName, responseEtag);
                        }
    ...
    private mapError(status: number, error: AxiosError): HttpException {
        ...
        const httpStatus = status === 429 ? HttpStatus.TOO_MANY_REQUESTS :
            status === 404 ? HttpStatus.NOT_FOUND :
                status >= 500 ? HttpStatus.SERVICE_UNAVAILABLE :
