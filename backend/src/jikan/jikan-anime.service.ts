...
export class JikanAnimeService {
    ...
    async getEpisodes(id: number): Promise<any> {
        ...
        const result = await this.executeWithRetry<{ data: Array<{ mal_id: number; title: string; episode: string; url: string }> }>(
            () => this.client.get(`anime/${id}/episodes`),
            cacheKey,
        );
    ...
    private mapStatus(jikanStatus: string): string {
        ...
        if (status.includes('currently') || status.includes('ongoing')) return 'ongoing';
        return 'completed';
    }
