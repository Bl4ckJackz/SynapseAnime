...
export class AdInsertionService {
  ...
  async shouldShowAd(
    ...
    const adConfig = await this.adService.getAdConfiguration(
  ...
  async getAdForUser(
    ...
    return await this.adService.getRandomAdForUser(
      userId,
      adType,
      'free_users',
    );
  ...
  private generateAdMarkers(
    adConfig: any,
  ): Array<{ time: number; adId: string }> {
