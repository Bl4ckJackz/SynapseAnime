...
export class AnimeStreamingService {
  ...
  async getStreamingSources(animeId: string, episodeNumber: number): Promise<StreamingSource[]> {
    try {
      ...
      console.error(`Error getting streaming sources for anime ${animeId}, episode ${episodeNumber}:`, error);
  ...
  async uploadInternalSource(file: Buffer, metadata: any): Promise<string> {
    ...
    console.log(`Uploading file for anime: ${metadata.animeId}, episode: ${metadata.episodeNumber}`);
  ...
  async validateDirectLink(url: string): Promise<boolean> {
    ...
    try {
      ...
      return url.startsWith('http') && (url.endsWith('.mp4') || url.endsWith('.m3u8'));
    } catch (error) {
  ...
  async addStreamingSource(animeId: string, episodeNumber: number, source: StreamingSource): Promise<void> {
    ...
    if (!episode) {
      throw new Error(`Episode ${episodeNumber} not found for anime ${animeId}`);
    }
  ...
  async getRecommendedEpisodes(animeId: string, currentEpisode: number, count: number = 3): Promise<Episode[]> {
  ...
  async getEpisodeWithFallbackSources(animeId: string, episodeNumber: number): Promise<{
    episode: Episode;
