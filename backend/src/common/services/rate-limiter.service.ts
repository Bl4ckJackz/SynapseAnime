...
export class RateLimiterService {
    ...
    async acquireToken(apiName: string): Promise<void> {
        const config = this.configs[apiName] || { requestsPerSecond: 5, requestsPerMinute: 100 };
        ...
        while (bucket.tokens < 1 || bucket.minuteTokens < 1) {
            const waitTime = bucket.tokens < 1
                ? Math.ceil((1 - bucket.tokens) * 1000 / config.requestsPerSecond)
                : Math.ceil((1 - bucket.minuteTokens) * 60000 / config.requestsPerMinute);

            this.logger.warn(`Rate limit reached for ${apiName}, waiting ${waitTime}ms`);
    ...
    getStats(apiName: string): { tokensRemaining: number; minuteTokensRemaining: number } | null {
