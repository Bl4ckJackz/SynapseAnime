...
export class AnimeUnitySource implements AnimeSource {
    ...
    private readonly consumetUrl = process.env.CONSUMET_API_URL || 'http://localhost:3002';
    ...
    async getAnimeList(filters: AnimeFilters): Promise<PaginatedResult<Anime>> {
        try {
            ...
            const animes: Anime[] = results.map((item: any) => ({
                id: item.id,
                title: item.title,
                coverUrl: item.image,
                description: '',
    ...
    async getAnimeById(id: string): Promise<Anime | null> {
        try {
            ...
            return {
                ...
                genres: data.genres || [],
                status: data.status === 'Completed' ? AnimeStatus.COMPLETED : AnimeStatus.ONGOING,
        ...
        try {
            ...
            for (const quality of preferredQualities) {
                const source = sources.find((s: any) =>
                    ...
                    s.url && !s.url.includes('youtube.com') && !s.url.includes('youtube-nocookie.com')
                );
                if (source) return source.url;
            }

            // Return first available non-YouTube source
            const validSource = sources.find((s: any) =>
                s.url && !s.url.includes('youtube.com') && !s.url.includes('youtube-nocookie.com')
            );
            if (validSource) {
