...
export class HiAnimeSource implements AnimeSource {
  ...
  readonly description =
    'Streaming from HiAnime via aniwatch-api with reliable M3U8 streams';
  ...
  async getAnimeList(filters: AnimeFilters): Promise<PaginatedResult<Anime>> {
    try {
      ...
      const animes: Anime[] = data.data.animes.map(
        (item: any) =>
          ({
            id: item.id,
            title: item.name,
      ...
      return {
        ...
        status:
          moreInfo?.status === 'Finished Airing'
            ...
            : AnimeStatus.ONGOING,
        releaseYear: moreInfo?.aired
          ? parseInt(moreInfo.aired.split(' ').pop()) || 0
          : 0,
