...
export class HiAnimeController {
    ...
    async getAnimeDetails(@Param('id') id: string) {
        try {
            ...
            console.error(`Error fetching details for anime ID ${id} from HiAnime:`, error);
            throw new NotFoundException(`Anime with ID ${id} not found on HiAnime`);
        }
    }
    ...
    async getAnimeEpisodes(@Param('animeId') animeId: string) {
        try {
            ...
            console.error(`Error fetching episodes for anime ID ${animeId} from HiAnime:`, error);
            return [];
        }
    }
    ...
    async getEpisodeStreamingLinks(@Req() request: Request) {
        ...
        let episodeId = decodeURIComponent(fullPath.replace('/anime/hianime/episode/', ''));
        ...
        try {
            ...
            console.error(`Error fetching episode streaming links for ID ${episodeId} from HiAnime:`, error);
            throw new NotFoundException(`Episode with ID ${episodeId} not found on HiAnime`);
        }
    }
}
